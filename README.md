# STOCK Pro

Application Next.js (gestion de stock).

**Dépôt :** [github.com/Artifique/StockPro](https://github.com/Artifique/StockPro)

## Démarrage

```bash
npm install
npm run dev
```

L’app est servie sur **http://localhost:3000**.

### Port 3000 déjà occupé (`EADDRINUSE`)

Voir **[docs/dev.md](docs/dev.md)** : libérer le port sous Windows ou lancer `npm run dev:3001`.

## Scripts utiles

| Commande | Description |
|----------|-------------|
| `npm run dev` | Serveur de dev sur le port 3000 |
| `npm run dev:3001` | Serveur de dev sur le port 3001 |
| `npm run build` | Build production |
| `npm run lint` | ESLint |
| `npm run typecheck` | Vérification TypeScript |
| `npm run start` | Démarrage production (serveur standalone) |
